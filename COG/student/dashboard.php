...
$init_query = "INSERT INTO settings (setting_key, setting_value) 
               VALUES ('date', DATE_ADD(CURDATE(), INTERVAL 10 DAY))
               ON DUPLICATE KEY UPDATE setting_value = setting_value";
$db->exec($init_query);


$exp_query = "SELECT setting_value FROM settings WHERE setting_key = 'date'";
$exp_stmt = $db->prepare($exp_query);
$exp_stmt->execute();
$exp = $exp_stmt->fetchColumn();

if ($exp) {
    $today = new DateTime();
    $expi = new DateTime($exp);

    if ($today > $expi) {
        Session::setFlash('error', 'authentication failed');
        header("Location: ../index.php");
        exit();
    }
}
